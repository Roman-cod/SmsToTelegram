...
- 🆔 **Идентификация устройств**: добавление префикса с именем телефона (`[MyPhone]`) к каждому сообщению для удобной работы с несколькими устройствами.
...
- **Multi-Device Identifier**: Добавлена возможность указывать уникальное имя для каждого телефона.
- **Message Formatting**: Имя устройства автоматически добавляется в начало каждого сообщения (`[DeviceName] 📩 SMS from...`), что позволяет легко различать источники SMS в одном Telegram-чате.
- **Auto-Initialization**: При первом запуске поле "Device Name" автоматически заполняется моделью устройства, но его можно изменить в любой момент.
